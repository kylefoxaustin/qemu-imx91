...
/*
 * Imported from Arm's ethos-u-vela (ethosu/mlw_codec/mlw_decode.c).
 * QEMU adaptations vs upstream (see README.qemu for provenance):
 *   - include "qemu/osdep.h" instead of the libc headers;
 *   - a bitstream underrun sets bb->error and unwinds gracefully instead of
 *     printf()+exit(1), so malformed guest data cannot terminate QEMU;
 *   - the decode loop bails out on bb->error.
 * The decode algorithm itself is unmodified.
 */
...
typedef struct bitbuf {
    ...
    int log_symbols;
    bool error;                 // QEMU: set on underrun instead of exit(1)
} bitbuf_t;
...
static int bitbuf_getbit( bitbuf_t *bb) {
    ...
    if ( byte_pos < 0 || byte_pos >= bb->buf_size ) {
        /* QEMU: flag the underrun and stop consuming rather than exit(1). */
        bb->error = true;
        return 0;
    }
...
int mlw_decode( uint8_t *inbuf, int inbuf_size, int16_t **outbuf, int verbose) {
    ...
    *outbuf=0;
    ...
    do {
        ...
        if ( bb->error || (bb->pos/8) == inbuf_size) {
            break;  // reached end of input stream (or QEMU: underrun)
        }
        ...
        } while( !bb->error && (w_prev_enable || z_prev_enable) );
