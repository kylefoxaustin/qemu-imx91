...
#
# In-guest build CORPUS (i.MX 91): build REAL upstream projects natively on the
# A55 with the gcc rootfs and run each project's OWN test suite. Beyond run-gcc.sh's
# toy programs - it proves the emulated i.MX 91 is a working development host:
# download -> ./configure / make -> `make test` passes, all in the guest.
#
# Adapted from the i.MX 95 harness. 91 delta: NO System Manager / M33; rootfs on
# the SD bus (-drive if=sd) = /dev/mmcblk0.
#
...
#
# Tarballs are fetched host-side (cached) and staged into the rootfs, so the guest
# needs no network. SKIPs if docker / mke2fs / boot assets / curl are missing. Real
# builds under TCG are slow - the default timeout is generous.
set -u
...
echo "=== IN-GUEST REAL BUILDS (i.MX 91, native gcc $($GCC -dumpversion), -j$J) ==="
...
# ---- build the ext4 root image + boot (NO System Manager) -------------------
...
echo "============ IN-GUEST REAL BUILDS (i.MX 91) ============"
