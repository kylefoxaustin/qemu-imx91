 ...
 *
 * i2clink - i2c-dev oracle for the i.MX 91 I2C board-to-board link
...
#define LINK_ADDR 0x42          /* the i2c-link target address (fixed) */
...
int main(int argc, char **argv)
{
    ...
    const char *payload = argc > 4 ? argv[4] :
                          "IMX91-I2C-LINK-payload-0123456789";
    ...
    fd = open(dev, O_RDWR);
    if (fd < 0) {
        printf("I2CLINK:FAIL:open %s (%s)\n", dev, strerror(errno));
        return 1;
    }

    if (!strcmp(role, "send")) {
        if (i2c_xfer(fd, LINK_ADDR, 0, (unsigned char *)payload, plen) < 0) {
    ...
    while (got < (int)sizeof(collected) && time(NULL) - start < 45) {
        int n = i2c_xfer(fd, LINK_ADDR, 1, rx, sizeof(rx));

        if (n < 0) {
            printf("I2CLINK:FAIL:recv xfer errno=%d\n", errno);
            return 1;
        }
        ...
        for (i = 0; i + plen <= got; i++) {
            if (!memcmp(collected + i, payload, plen)) {
                printf("I2CLINK:PASS: %d bytes crossed the I2C link\n",
                       plen);
                return 0;
            }
        }
