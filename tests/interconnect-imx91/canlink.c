...
/*
 * canlink - a two-role SocketCAN oracle for the i.MX 91 board-to-board CAN
 * link test (tests/interconnect-imx91/run-can.sh).
 *
 * Copyright (c) 2026, Kyle Fox
 *
 * Unlike a one-instance FlexCAN loopback self-test, this splits into send/recv
 * roles so two QEMU i.MX 91 instances - each a FlexCAN on a can-bus joined to
 * the other by a can-host-chardev socket bridge - can prove a CAN frame crosses
 * byte-exact between guests:
 *
 ...
 * across a window and the receiver watches a window - any match is a PASS
 * (CANLINK:PASS), like the SPI-link oracle. Self-contained (raw rtnetlink
 * bring-up, no iproute2/can-utils); static-link for aarch64.
 */
