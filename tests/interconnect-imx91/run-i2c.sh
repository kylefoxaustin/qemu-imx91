#!/bin/sh
#
# i.MX 91 <-> i.MX 91 I2C board-to-board interconnect.
#
# Boots two imx91-11x11-evk guests. Each board's LPI2C3 master gets an i2c-link
...
ADDR=${ADDR:-0x42}
PAYLOAD=${PAYLOAD:-IMX91-I2C-LINK-payload-0123456789}
MEM=${MEM:-2G}
TMO=${TMO:-200}
...
SOCK=${SOCK:-$(mktemp -u /tmp/imx91-i2clink.XXXXXX.sock)}
...
echo "== booting i.MX 91 RECEIVER (i2c-link socket listen) =="
boot "$RECV_IRD" "-chardev socket,id=i2cl,path=$SOCK,server=on,wait=off" "$RLOG"; SPID=$!
sleep 2
echo "== booting i.MX 91 SENDER (i2c-link socket connect) =="
...
if grep -aq 'I2CLINK:PASS' "$RLOG"; then
    echo "PASS: payload crossed LPI2C<->i2c-link<->socket<->i2c-link<->LPI2C byte-exact between two i.MX 91 guests"
    exit 0
fi
