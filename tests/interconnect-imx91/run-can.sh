...
#
# CAN INTERCONNECT (i.MX 91): prove two QEMU i.MX 91 instances pass a real CAN
...
# by can-host-chardev (net/can/can_host_chardev.c, the fleet-shared generic CAN
# transport) over a chardev socket - so no host-kernel vcan/SocketCAN (root) is
# needed, unlike can-host-socketcan. The stock i.MX 91 EVK DT already enables
# flexcan2 (can@425b0000), so no DT overlay is needed; both machine can-buses are
# wired to 'cb' (canbus0=cb,canbus1=cb) so whichever node the guest enumerates as
# can0 is on the bridged bus. The CAN stack loads as modules from the BSP.
set -u
...
BSP_MOD=${BSP_MOD:-$HOME/Documents/nxp/linux/imx-yocto-bsp/build-imx91/tmp/work}
MEM=${MEM:-2G}
TMO=${TMO:-200}
...
# locate the CAN modules in the BSP (kernel-version-specific paths)
declare -A KO
for m in can can-dev can-raw flexcan; do
    p=$(find "$BSP_MOD" -name "$m.ko" 2>/dev/null | head -1)
    [ -n "$p" ] || skip "missing CAN module $m.ko (set BSP_MOD=)"
    KO[$m]=$p
done

SOCK=${SOCK:-$(mktemp -u /tmp/imx91-canlink.XXXXXX.sock)}
...
boot() {                    # $1=initrd  $2=chardev-args  $3=logfile
    timeout --signal=KILL "$TMO" "$QEMU" -M imx91-11x11-evk -audio driver=none -smp 1 -m "$MEM" \
...
echo "== booting i.MX 91 RECEIVER (can-host-chardev socket listen) =="
boot "$RECV_IRD" "-chardev socket,id=canl,path=$SOCK,server=on,wait=off" "$RLOG"; SPID=$!
sleep 3
echo "== booting i.MX 91 SENDER (can-host-chardev socket connect) =="
...
if grep -aq 'CANLINK:PASS' "$RLOG"; then
    echo "PASS: CAN frame crossed FlexCAN<->can-bus<->can-host-chardev<->socket<->...<->FlexCAN byte-exact between two i.MX 91 guests"
    exit 0
fi
